...
function jal_install() {
  ...
  `suppliar_key` varchar(255) NOT NULL,
  `rtbs_domain` varchar(255) NOT NULL,
  `promo_code` int(11) NOT NULL COMMENT '0=show,1=hide',
  `success_url` varchar(255) NOT NULL,
  `tearms_cond` text NOT NULL,
  `tour_keys` varchar(255) NOT NULL COMMENT 'seperated by comma',
...
function adminSettings()
{
  ...
  $checkUpdate = $wpdb->update(
    ...
    'suppliar_key' => stripcslashes($_POST['suppliar_key']),
    ...
    'tearms_cond' => stripcslashes($_POST['t_c']),
...
<td><input name="api_key" type="text" id="blogname" value="<?php echo $getRowval->api_key; ?>" class="regular-text">
...
<td><input name="suppliar_key" type="text" id="blogdescription" aria-describedby="tagline-description" value="<?php echo $getRowval->suppliar_key; ?>" class="regular-text">
...
<td><input name="rtbs_domain" type="text" id="blogdescription" aria-describedby="tagline-description" value="<?php echo $getRowval->rtbs_domain; ?>" class="regular-text">
...
<td><input name="promo_code" type="checkbox" id="blogdescription" aria-describedby="tagline-description" <?php echo ($getRowval->promo_code)==0 ? 'checked' : '' ?> class="regular-checkbox" value="0">
...
<td><input name="remaining" type="checkbox" id="blogdescription" aria-describedby="tagline-description" class="regular-checkbox" <?php echo ($getRowval->remaining)==0 ? 'checked' : '' ?> value="0">
<p class="description">Show/Hide</p>
</td>
</tr>
...
<td><input name="success_url" type="text" id="blogdescription" aria-describedby="tagline-description" value="<?php echo $getRowval->success_url; ?>" class="regular-text">
...
<td><input name="page_title" type="text" id="blogdescription" aria-describedby="tagline-description" value="<?php echo $getRowval->page_title; ?>" class="regular-text">
...
<td><input name="section_title" type="text" id="blogdescription" aria-describedby="tagline-description" value="<?php echo $getRowval->section_title; ?>" class="regular-text">
...
<td><input name="title_first_page" type="text" id="blogdescription" aria-describedby="tagline-description" value="<?php echo $getRowval->title_first_page; ?>" class="regular-text">
...
<td><!--<textarea name="t_c" type="text" id="t_c" aria-describedby="tagline-description" value="" class="regular-textarea"></textarea>-->
<?php wp_editor( $getRowval->tearms_cond, 't_c', $settings = array( 'media_buttons' => false ) ); ?>
</td>
</tr>
...
<td>
	<textarea name="css_style" rows="30" cols="50" id="blogname" class="large-text code"><?php echo ($getRowval->css_style<>'' ? $getRowval->css_style : ''); ?></textarea>
...
<div class="col-md-3 col-sm-3 col-xs-3 <?php echo ($_POST['hd_step']!='2' && $_POST['hd_step']!='3' && $_POST['hd_step']!='4') ? 'selected' : '' ?>">
...
<?php if($_POST['hd_step']==2 || $_POST['hd_step']==3 || $_POST['hd_step']==4){ ?>
  ...
  <h4>Selected Date & Time: <span style="color:#000;"><?php echo date('l dS F Y h:i a',strtotime($_POST['hd_tour_date_time'])); ?></span></h4>
  <?php } else { ?>
		<?php
...
<h4>Showing: <?php echo (isset($_REQUEST['tdate'])) ? date('l dS F Y',strtotime($_REQUEST['tdate'])) : date('l dS F Y',strtotime($tomorrowDate)); ?></h4>
<p>
...
<p><i class="fa fa-calendar" aria-hidden="true"></i>  <input onchange="selectDate(this.value)" type="text" placeholder="Change Date" id="datepicker" value="<?php echo (isset($_REQUEST['tdate'])) ? $_REQUEST['tdate'] : ''; ?>"></p>
<?php } ?>
...
if($_POST['hd_step']=='4'){
	...
	$supplier_key = $getRowval->suppliar_key; //a Demonstration Supplier Key. Replace with a Supplier Key as provided to you by Whytewaters
...
<div class="row rtbs-tours-step-<?php if($_POST['hd_step']=='2') { echo '2'; } elseif($_POST['hd_step']=='3') { echo '3'; } elseif($_POST['hd_step']=='4') { echo '4'; } else { echo '1'; } ?>">
<?php
            ...
            //if($session->is_open()){
              ...
              <form action="" method="post">
                ...
                <input type="hidden" name="hd_step" value="2">
								<input type="hidden" name="hd_remaining" value="<?php echo $session->get_remaining(); ?>"/>
                <input type="hidden" name="hd_tour_key" value="<?php echo $tour->get_tour_key(); ?>">
                <input type="hidden" name="hd_date" value="<?php echo (isset($_REQUEST['tdate'])) ? $_REQUEST['tdate'] : date('Y-m-d'); ?>">
                <input type="hidden" name="hd_tour_name" value="<?php echo $tour->get_name(); ?>">
                <input type="hidden" name="hd_tour_date_time" value="<?php echo $session->get_datetime(); ?>">
                <button <?php echo ($session->is_open()) ? '' : 'disabled' ?> class="btn btn-primary" type="submit" name="button"><?php echo $session->get_state(); ?></button>
							</p>
              </form>
...
<form onSubmit="return validd()" class="form-horizontal" action="" method="post">
  <fieldset>
      <p style="font-size: 18px;background-color: #ecf0f1;padding: 10px;" class=""><?php echo ($expsectionTitle[0]<>'' ? $expsectionTitle[0] : 'Number of People'); ?> </p>
      <?php
  ...
  <div class="form-group">
    <label for="select" class="col-lg-2 col-md-2 col-sm-2"><?php echo $price_cat->get_name(); ?></label>
    <div class="col-lg-10 col-md-10 col-sm-10">
		 <div class="col-md-8 col-sm-8 col-xs-8 col-lg-8">

      <input type="hidden" name="<?php echo 'rate'.$z; ?>" id="<?php echo 'rate'.$z; ?>" class="" value="<?php echo $price_cat->get_rate();?>">
      <select class="form-control nPeople" name="pr_ice[]" id="<?php echo $z; ?>">
      ...
      <?php echo '$'.$price_cat->get_rate(); ?>
			...
			<input type="hidden" name="hd_price_name[]" value="<?php echo $price_cat->get_name(); ?>">
			<input type="hidden" name="hd_price_rate[]" value="<?php echo $price_cat->get_rate(); ?>">
    </div>
  </div>
  <?php $z++; ?>
  <?php } ?>
...
<p style="font-size: 18px;background-color: #ecf0f1;padding: 10px;" class=""><?php echo ($expsectionTitle[1]<>'' ? $expsectionTitle[1] : 'Your Details'); ?> </p>
...
<p style="font-size: 18px;background-color: #ecf0f1;padding: 10px;" class="">Pickup </p>
 ...
 <div class="form-group">
   ...
   <div class="col-lg-12">
     <select class="form-control" name="pickup_key">
     	...
     	<option value="<?php echo $pkup->get_pickup_key(); ?>"><?php echo ($pkup->get_name()=='' ? 'No Pickup available' : $pkup->get_name().' - '.$pkup->get_place().' - '.date('h:i a' ,strtotime($_POST['hd_tour_date_time'].' -'.$pkup->get_minutes().' minutes'))); ?></option>
			<?php } ?>
		 </select>
...
<div class="hidden_hd">
  <input type="hidden" name="hd_step" value="3">
	<input type="hidden" name="hd_remaining" value="<?php echo $_POST['hd_remaining']; ?>"/>
...
<div class="form-group">
  <div class="col-lg-10">
<button type="submit" onclick="confirmm()" class="btn btn-primary pull-right" name="button">NEXT</button>
</div>
</div>
...
<form onSubmit="return validd()" class="form-horizontal" action="" method="post">
  ...
  <div class="form-group">
    ...
    <div class="col-lg-10 col-md-10 col-sm-10">
      ...
      <select class="form-control nPeople" name="pr_ice[]" id="<?php echo $z; ?>">
          <?php for($i=0;$i<=20;$i++){ ?>
        <option <?php echo ($_POST['pr_ice'][$x]==$i ? 'selected' : ''); ?> value="<?php echo $i; ?>"><?php echo $i; ?></option>
      <?php } ?>
      </select>
		</div>
		<div class="col-md-4 col-sm-4 col-xs-4 col-lg-4">
...
<div class="form-group">
  <label for="inputEmail" class="col-lg-3">First Name</label>
  <div class="col-lg-9">
    <input class="form-control" type="text" name="fname" value="<?php echo (isset($_POST['fname']) ? preg_replace('/\\\\/', '', $_POST['fname']) : ''); ?>">
...
<div class="form-group">
  <label for="inputEmail" class="col-lg-3">Last Name</label>
  <div class="col-lg-9">
    <input class="form-control" type="text" name="lname" value="<?php echo (isset($_POST['lname']) ? preg_replace('/\\\\/', '', $_POST['lname']) : ''); ?>">
...
<div class="form-group">
  <label for="inputEmail" class="col-lg-3">Phone</label>
  <div class="col-lg-9">
    <input class="form-control" type="number" name="phone" value="<?php echo (isset($_POST['phone']) ? $_POST['phone'] : ''); ?>">
...
<div class="form-group">
  <label for="inputEmail" class="col-lg-3">Promo Code</label>
  <div class="col-lg-9">
    <input class="form-control" type="text" name="promo" value="<?php echo (isset($_POST['promo']) ? $_POST['promo'] : ''); ?>">
...
<tr>
	...
	<td>
		...
		<input type="hidden" name="fname" value="<?php echo preg_replace('/\\\\/', '', $_POST['fname']); ?>">
		<input type="hidden" name="lname" value="<?php echo preg_replace('/\\\\/', '', $_POST['lname']); ?>">
	</td>
</tr>
...
<tr>
	<td colspan="2">
		...
		<p class="t_c"><?php echo $getRowval->tearms_cond; ?></p>
		<input type="checkbox" name="tandc" id="chk_conf" value="0"> I have read and accept the Terms and Conditions.<br/><br/>
	</td>
</tr>
 ...
 <button id="confirm_pay" disabled type="submit" class="btn btn-primary pull-right" name="confirm_payment">Confirm & Make Payment</button>
